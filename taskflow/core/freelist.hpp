...
class NodeStack {
  ...
  void bulk_push(I first, size_t N) {
    ...
    for(size_t i=0; i<N-1; ++i) {
      first[i]->_next = first[i+1];
    }
    first[N-1]->_next = nullptr;
    

    TaggedPointer curr_head = head.load(std::memory_order_relaxed);

    while (true) {
      first[N-1]->_next = curr_head.ptr;
      // Create a new tagged pointer with an incremented tag
  ...
  Node* steal() {
    ...
    while (curr_head.ptr != nullptr) {
      TaggedPointer next_head = {curr_head.ptr->_next, curr_head.tag + 1};

      if (head.compare_exchange_weak(curr_head, next_head,
                                     std::memory_order_release,
                                     std::memory_order_acquire)) {
        curr_head.ptr->_next = nullptr;
...
/*class NodeStack {
  ...
  Node* unpack(uintptr_t val) const {
    return reinterpret_cast<Node*>(val & ADDR_MASK);
  }

  // Helper to increment the tag and pack a new pointer
  uintptr_t pack(Node* ptr, uintptr_t old_val) const {
