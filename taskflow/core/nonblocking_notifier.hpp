...
// ----------------------------------------------------------------------------
// NonblockingNotifierV3
// ----------------------------------------------------------------------------

#ifdef __cpp_lib_atomic_wait
class NonblockingNotifierV3 {
  ...
  explicit NonblockingNotifierV3(size_t N) : _state(kStackMask), _waiters(N) {
    assert(N < ((1 << kWaiterBits) - 1));
  }
  ...
  void _notify() {
    ...
    for (;;) {
      ...
      if (_state.compare_exchange_weak(state, newstate, std::memory_order_acq_rel)) {
        ...
        if(!notifyAll) _state.notify_one();
        else _state.notify_all();
  ...
  NonblockingNotifierV3(const NonblockingNotifierV3&) = delete;
  void operator=(const NonblockingNotifierV3&) = delete;
};
#endif
