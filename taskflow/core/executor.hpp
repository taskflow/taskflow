...
#ifdef TF_ENABLE_CUDA
// Constructor
inline Executor::Executor(size_t N, size_t M) : _VICTIM_BEG{0},
                                                _VICTIM_END{N + M - 1},
                                                _MAX_STEALS{(N + M + 1) << 1},
                                                _MAX_YIELDS{100},
                                                _workers{N + M},
...
#else
// Constructor
inline Executor::Executor(size_t N) : _VICTIM_BEG{0},
                                      _VICTIM_END{N - 1},
                                      _MAX_STEALS{(N + 1) << 1},
                                      _MAX_YIELDS{100},
                                      _workers{N},
...
inline void Executor::_spawn(size_t N, Domain d) {
  ...
  for (size_t i = 0; i < N; ++i, ++id) {
    ...
    _threads.emplace_back(fn, std::ref(_workers[id]));
  }
}
