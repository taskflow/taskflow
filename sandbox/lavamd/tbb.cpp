...
void  tbb_kernel(par_str par, 
  ...
  parallel_for(0, (int)dim.number_boxes, 1, [&par, dim, box, rv, qv, fv](int l){
...
std::chrono::microseconds measure_time_tbb(unsigned num_threads) {
  ...
  tbb_kernel(par_cpu, dim_cpu, box_cpu,	rv_cpu,	qv_cpu,	fv_cpu, num_threads);
