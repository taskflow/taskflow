...
int main() {
  ...
  taskflow.emplace([&](tf::cudaFlowCapturer& capturer){
    ...
    auto h2dA = blas->copy(dA, hA.data(), hA.size()).name("copy A");
    auto h2dB = blas->copy(dB, hB.data(), hB.size()).name("copy B");
    auto trsv = blas->c_trsv(
      CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, CUBLAS_DIAG_UNIT, 
      N, dA, N, dB, 1
    ).name("trsv");
    auto d2h = blas->copy(res.data(), dB, res.size()).name("copy result");
