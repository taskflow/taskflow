WORK_DIR=/home/xiongzc/taskflow/scripts
EXEC=/home/xiongzc/taskflow/build/benchmarks/data_pipeline_dev

if [ ! -d "$WORK_DIR/normal_vs_efficient" ]; then
  mkdir $WORK_DIR/normal_vs_efficient
fi

cd $WORK_DIR
PLIST="ssss sppp ssssssss sppppppp ssssssssssssssss sppppppppppppppp"
NLIST="4 8 16 24"
for p in $PLIST; do
  if [ -e "normal_vs_efficient/$p.txt" ]; then
    rm "normal_vs_efficient/$p.txt"
  fi
  for n in $NLIST; do
    for m in normal efficient; do
      $EXEC -t $n -r 3 -m $m -l $n -p $p >> "normal_vs_efficient/$p.txt"
    done
  done
done 